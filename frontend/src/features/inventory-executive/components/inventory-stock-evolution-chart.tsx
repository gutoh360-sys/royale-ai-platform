...
export function InventoryStockEvolutionChart({ data, "data-testid": testId }: StockEvolutionChartProps) {
  const [tooltip, setTooltip] = useState<TooltipData | null>(null);

  const { path, dots, ticksY, ticksX } = useMemo(() => {
    if (data.length === 0) return { path: "", dots: [], ticksY: [], ticksX: [] };

    const values = data.map((d) => d.itemsInStock);
    ...
    const pts = data.map((d, i) => ({
      x: PAD.left + i * stepX,
      y: PAD.top + IH - ((d.itemsInStock - min) / range) * IH * 0.9 - IH * 0.05,
      value: d.itemsInStock,
      date: d.date,
    }));
    ...
    return { path: p, dots: pts, ticksY: tY, ticksX: tX };
  }, [data]);
  ...
  return (
    <Card data-testid={testId}>
      <CardHeader className="pb-1">
        <CardTitle className="text-sm font-medium">Evolucao do Estoque</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="relative">
          <svg
            ...
            role="img"
            aria-label="Grafico de evolucao do estoque nos ultimos 30 dias"
            style={{ maxHeight: 280 }}
          >
            ...
            {dots.map((pt, i) => (
              <circle
                ...
                onMouseEnter={() => handleMouseEnter({ x: pt.x, y: pt.y, date: pt.date, value: pt.value, label: fmtNumber(pt.value) })}
                onMouseLeave={handleMouseLeave}
                role="graphics-symbol"
                aria-label={`${fmtDateFull(pt.date)}: ${fmtNumber(pt.value)} itens`}
          ...
          {tooltip && (
            ...
            >
              ...
              <p className="text-muted-foreground">{tooltip.label} itens</p>
            </div>
          )}
        </div>
      </CardContent>
