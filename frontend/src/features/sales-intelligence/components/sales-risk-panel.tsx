...
interface SalesRiskPanelProps {
  opportunities: SalesOpportunity[]
}

export function SalesRiskPanel({ opportunities }: SalesRiskPanelProps) {
  const groups = { critical: 0, high: 0, medium: 0, low: 0 } as Record<string, number>
  for (const o of opportunities) groups[o.priority]++
        ...
        <div className="space-y-2">
          {Object.entries(groups).map(([level, count]) => (
            <div key={level} className="flex items-center gap-3">
              <span className={cn("w-16 text-xs font-medium", priorityConfig[level]?.color)}>
