...
import type { ProductsInsightInput } from "./inputs/products-insight-input"
...
function buildId(idx: number): string {
  return `products-insight-${idx}`
}

export class ProductsExecutiveInsightBuilder
  implements ExecutiveInsightBuilder<ProductsInsightInput>
{
  readonly module = ExecutiveInsightCategory.PRODUCTS

  build(input: ProductsInsightInput, context: BuilderContext): readonly ExecutiveInsight[] {
    const insights: ExecutiveInsight[] = []

    for (let i = 0; i < input.existingInsights.length; i++) {
      const src = input.existingInsights[i]
      insights.push({
        key: buildInsightKey("products", src.fact),
        id: buildId(i),
        module: "products",
    ...
    for (let i = 0; i < input.existingRecommendations.length; i++) {
      ...
      insights.push({
        key: buildInsightKey("products", src.action),
