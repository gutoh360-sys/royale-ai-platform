...
import type { ChartDataPoint } from "@/features/financial-executive/mocks/chart-mock";
...
export function FinancialCashFlowChart({ data, "data-testid": testId }: CashFlowChartProps) {
  ...
  const { path, ticksY, ticksX } = useMemo(() => {
    ...
    const pts = data.map((d, i) => ({
      x: PAD.left + i * stepX,
      y: PAD.top + IH - ((d.cashFlow - min) / range) * IH * 0.9 - IH * 0.05,
      value: d.cashFlow,
      date: d.date,
    }));
      ...
      <CardContent>
        <div className="relative">
          <svg
            ...
            role="img"
            aria-label="Grafico de fluxo de caixa nos ultimos 30 dias"
            style={{ maxHeight: 280 }}
          >
            ...
            {path && (
              <path d={path} fill="none" className="stroke-success" strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
            )}

            {data.map((d, i) => {
              const stepX2 = IW / (data.length - 1);
              const x = PAD.left + i * stepX2;
              const values = data.map((dd) => dd.cashFlow);
              const max2 = Math.max(...values);
              const min2 = Math.min(...values);
              const range2 = max2 - min2 || 1;
              const y = PAD.top + IH - ((d.cashFlow - min2) / range2) * IH * 0.9 - IH * 0.05;
              return (
                <circle
                  key={i}
                  cx={x}
                  cy={y}
                  r={3}
                  className="fill-success opacity-0 hover:opacity-100 transition-opacity duration-150"
                  onMouseEnter={() => handleMouseEnter({ x, y, date: d.date, value: d.cashFlow })}
                  onMouseLeave={handleMouseLeave}
                  role="graphics-symbol"
                  aria-label={`${fmtDateFull(d.date)}: ${fmtCurrency(d.cashFlow)}`}
