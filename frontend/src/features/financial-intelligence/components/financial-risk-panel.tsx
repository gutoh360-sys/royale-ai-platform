...
export function FinancialRiskPanel({ insights }: FinancialRiskPanelProps) {
  const groups = { critical: 0, high: 0, medium: 0, low: 0 } as Record<string, number>
  for (const i of insights) groups[i.financialRisk]++
            ...
            <div key={level} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-2 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{ width: `${insights.length > 0 ? (count / insights.length) * 100 : 0}%` }}
                />
              </div>
